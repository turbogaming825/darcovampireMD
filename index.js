...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage(`917373104400@s.whatsapp.net`, {text: `*Report Bot:* Someone Called Bot`})
    await sleep(8000)
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./Darco")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/7a61633e6bda2ea470b80.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
